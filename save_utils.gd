...
## Clones object properties lised in PERSIST_PROPERTIES or PERSIST_PROPERTIES2
## arrays. Arrays and Dictionaries are duplicated (deep == true). However,
## Objects will be set without duplication.[br][br]
##
## This method is not used by [IVTreeEncoder]. It uses the same persist
## property lists however, so may be useful.
...
## Generates an array of object properties lised in PERSIST_PROPERTIES or
## PERSIST_PROPERTIES2 arrays. Using this function together with
## [code]set_persist_properties()[/code] is equivilent to [code]clone_persist_properties()[/code]
